...
static inline QUADRATURE_STATE GenerateQuadratureState(bool encoder1, bool encoder2)
{
	return (QUADRATURE_STATE)((encoder1 ? 0b01 : 0b00) + (encoder2 ? 0b10 : 0b00));
}

static inline int16_t GeneratePositionChange(QUADRATURE_STATE oldState, QUADRATURE_STATE newState)
{
	...
	{
		case QUAD_LO_LO:
			if (newState == QUAD_LO_HI) return -1;
			if (newState == QUAD_HI_LO) return 1;
			Assert(0);
			break;

		case QUAD_HI_LO:
			if (newState == QUAD_LO_LO) return -1;
			if (newState == QUAD_HI_HI) return 1;
			Assert(0);
			break;
...
void MotorInit(void)
{
	...
	darkSkyContext.motor1.quadratureState = GenerateQuadratureState(ioport_get_pin_level(MOTOR_DEC_ENC_1), ioport_get_pin_level(MOTOR_DEC_ENC_2));
	darkSkyContext.motor2.quadratureState = GenerateQuadratureState(ioport_get_pin_level(MOTOR_RA_ENC_1), ioport_get_pin_level(MOTOR_RA_ENC_2));
...
void encoder_handler_dec(const uint32_t id, const uint32_t index)
{
	...
	QUADRATURE_STATE newState = GenerateQuadratureState(
		pio_get(PIOC, PIO_TYPE_PIO_INPUT, PIO_PC17),
		pio_get(PIOC, PIO_TYPE_PIO_INPUT, PIO_PC19));
	
	darkSkyContext.motor1.position += GeneratePositionChange(darkSkyContext.motor1.quadratureState, newState);
...
void encoder_handler_ra(const uint32_t id, const uint32_t index)
{
	...
	QUADRATURE_STATE newState = GenerateQuadratureState(
		pio_get(PIOC, PIO_TYPE_PIO_INPUT, PIO_PC16),
		pio_get(PIOC, PIO_TYPE_PIO_INPUT, PIO_PC18));
	
	darkSkyContext.motor2.position += GeneratePositionChange(darkSkyContext.motor1.quadratureState, newState);
